...
public class HomeControllerTest {
	...
	public void testMakeOrder() throws Exception {
		...
		mockMvc.perform(
				post("/makeOrder").param("itemCheckBox", checkBoxSelected).flashAttr("itemWrapper", itemWrapper))
				.andExpect(forwardedUrl("paymentgateway")).andExpect(model().attributeExists("order"))
				.andExpect(model().attributeExists("finalItemList"));
	}
	...
	//@Test
	public void testErrorScenarionMakeOrder() throws Exception {
		
		final ItemWrapper itemWrapper = new ItemWrapper();
		itemWrapper.setItemList(JUnitConstants.ITEM_LIST);
		
		final String[] dummycheckBoxSelected = new String[]{"2","3"};
		
			
		mockMvc.perform(
				post("/makeOrder").flashAttr("itemCheckBox", dummycheckBoxSelected).flashAttr("itemWrapper", itemWrapper))
				.andExpect(forwardedUrl("homepage")).andExpect(model().attributeExists("error"));
	}
